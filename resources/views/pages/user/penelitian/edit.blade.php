        ...
        <div class="row">
            ...
            <div class="col-md-12">
                ...
                <div class="card card-primary">
                    ...
                    <!-- /.card-header -->
                    <!-- form start -->
                    <form action="{{route('kinerja-penelitian.update', $data->id)}}" method="POST" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <select name="penunjang_id" class="form-control" id="penunjang_id">
                                    ...
                                    <option value="{{$item->id}}" {{$item->id == $data->id_penelitian ? 'selected' : ''}}>{{$item->kode_penelitian}}. {{$item->nama_penelitian}}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group row">
                                <div class="col-md-6">
                                    ...
                                    <input type="date" name="tgl_mulai" id="tgl_mulai" value="{{$data->tgl_mulai}}" class="form-control">
                                </div>
                                <div class="col-md-6">
                                    <label for="tgl_selesai">Tanggal Selesai</label>
                                    <input type="date" name="tgl_selesai" id="tgl_selesai" value="{{$data->tgl_selesai}}" class="form-control">
                                </div>
                            </div>

                            <div class="form-group">
                                <label for="sks_penugasan">SATUAN KREDIT SEMESTER (SKS)</label>
                                <input type="text" class="form-control" name="sks_beban_kerja" id="sks_beban_kerja" value="{{old('sks_beban_kerja') ? old('sks_beban_kerja') : $data->sks_beban_kerja}}" required
                                    placeholder="Jumlah SKS">
                            </div>
                            <div class="form-group">
                                <label for="namafile">File Yang Sudah Diupload</label>
                                @foreach (json_decode($data->filenames) as $item){
                                    <a href="{{asset('/doc/penelitian/'.$item)}}" download="">Download</a>
                                }
                                @endforeach
                            </div>
                            <div class="form-group">
